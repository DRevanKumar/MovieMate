...
function CreatePost() {
    ...
    const getMovieById = async (imdbID) =>{
        try {
            ...
            const response = await fetch(`https://www.omdbapi.com/?apikey=7e316c84&i=${imdbID}`);
    ...
    const handleSearch = async () => {
      ...
      try {
        // Replace with your actual API URL and key
        const response = await fetch(`https://www.omdbapi.com/?apikey=eecc8976&s=${searchTerm}`);
        const data = await response.json();
  ...
  return (
    <div style={{"backgroundColor" : "rgba(0, 5, 19, 0.973)"}} className="min-h-screen flex items-center justify-center mt-20">
      <div className="w-full max-w-xl p-6 bg-white rounded-lg shadow-lg">
        ...
        {!hideForm && <button className="w-full py-2 mt-4 bg-blue-500 text-white rounded-lg hover:bg-black-600 transition">
          Post
        </button>}
      </div>
    </div>
